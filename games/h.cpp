// HKJC 5.9.1 欧皇MAX 彩蛋版 帖子质量评价生成器 
// 仅适用 Windows 用户，确保版本在 C++11 及以上
// 作者：一只小菜包/Dream_Country
// 版权所有：Blue space 旗下 HKJC 
// 本程序内核为 ACFJ-Plus ，并在此优化改进
// 停止更新为2023.12.15 
// 内测验证码：527yZ7scFGt08iM3
// 内测终结版 
// 2024.7-2024.8 发行版 
// 未经允许，禁止转载！ 
#include<bits/stdc++.h>	
...
#define END system("cls"),cout<<"未定义行为，已退出程序。",system(">nul 2>nul pause"),exit(0)
using namespace std;
...
const string version=" 5.9.1 欧皇MAX 彩蛋版 ";
const string title=R"(############################################
# H  H  K K   J   CCC   M M     A    X X   #
# HHHH  KK    J  C     M M M   AAA    X    #
# H  H  K K  JJ   CCC  M   M  A   A  X X   #
#                                          #
# Blue space版权所有 HKJC开发组编写        # 
############################################
)";
 
int op,vop;
double all;
string v,ret,opp[10]{""," JC 或疑似 JC ","无意义","恶意哗众取宠","违规","AFO"},g[10]{""," $\\color{#CFB53B}{S}$"," $\\color{#FF0000}{A}$"," $\\color{#e47833}{B}$"," $\\color{#bc1717}{C}$"," $\\color{#EAADEA}{D}$"};
void get(){cout<<"尊贵的欧皇会员，请您输入>>>";cin>>op;}
void getstring(){
  cout<<"尊贵的欧皇会员，请您输入>>>";
  char c;
  while((c=getchar())==' '||c=='\n') ;
...
int main(){
  Color(5);
  SetConsoleTitle("Mr.Hacker集成开发系统项目（HKJC） 5.9.1 欧皇MAX 彩蛋版 帖子质量评价生成器  作者：一只小菜包");
  SetWindowLongPtrA(GetConsoleWindow(),GWL_STYLE,GetWindowLongPtrA(GetConsoleWindow(),GWL_STYLE)&~WS_SIZEBOX&~WS_MAXIMIZEBOX&~WS_MINIMIZEBOX);
  cout<<title;
  Sleep(1000);
  int rmt=MessageBox(NULL,(LPCTSTR)TEXT("欢迎使用 HKJC欧皇MAX 产品！ 作者：一只小菜包  请问您是否使用？"), (LPCTSTR)TEXT("HKJC 欧皇MAX"), MB_YESNO);
  if(rmt==IDNO){
  	system("cls");
  	cout<<"那好吧！再见！";
  	return 0; 
  }
  int rct=MessageBox(NULL,(LPCTSTR)TEXT("欢迎使用 HKJC欧皇MAX 产品！ 本产品为全自动哦！"), (LPCTSTR)TEXT("HKJC 欧皇MAX"), MB_OK);
  system("cls");
  string sb;
  cout<<"请输入验证码：";
  cin>>sb;
  ...
  cout<<"请问需要宣传你的洛谷账号吗？（回复数字）\n【1】好\n【2】不需要\n\n\n"; 
  cout<<"尊贵的欧皇会员"<<bu<<"，请您输入>>>";
  string zhsz;
  cin>>zhsz;
  system("cls");
  if(zhsz=="1"){
  	cout<<"尊贵的欧皇会员"<<bu<<"，请您输入洛谷UID>>>";
  	cin>>zhsz; 
  	system("cls");
  	//[aaa](www)
  	ret+="\n###### 本程序由["+bu+"]"+"(https://www.luogu.com.cn/user/"+zhsz+")"+"生成\n\n";
  ...
  if(op<=5&&op>=1)
    ...
    getstring();
    string sa="",sb="";
    if((v.front()<='Z'&&v.front()>='A')||(v.front()<='z'&&v.front()>='a')||(v.front()<='9'&&v.front()>='1')) sa=" ";
    if((v.back()<='Z'&&v.back()>='A')||(v.back()<='z'&&v.back()>='a')||(v.front()<='9'&&v.front()>='1')) sb=" ";
    ret="本帖是"+sa+v+sb+"\n\n";
    system("cls");
  }
  else END;
  cout<<"是否取消引导？请确保你熟悉 HKJC 评价标准。（回复数字）\n【1】好\n【2】算了\n\n";
  get();
  if(op==1){
  	int pj; 
  	
    system("cls");
    cout<<"本帖需要超多方面评价，还是较简约评价？（回复数字）\n【1】超多\n【2】简约\n\n"<<endl;
    cout<<"尊贵的欧皇会员"<<bu<<"，请您输入>>>";
    cin>>pj;
    system("cls");
	if(pj==2){
		int po[10]{0,7,4,2,1};
    cout<<"该帖内容丰富度评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖内容丰富度评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖内容潮流度评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖内容潮流度评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖文本表达评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖文本表达评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖情感真实度评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖情感真实度评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖创新性评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖创新性评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=5;
	} 
    else if(pj==1) {
    ...
    ret+="该帖创新性评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖阅读后心情评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖阅读后心情评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖阅读后心得如何？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖阅读后心得价值"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖JC力度如何？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖JC力度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖JC水平如何？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖JC水平为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=9;
    ...
    ret+="该帖阅读后心得价值"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖无意义的程度为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖无意义的程度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖发表的意义程度为？（回复等级，如 A，最高 S，最低 C）注意越没意义等级越高！ \n\n";
    getstring();
    ret+="该帖发表的意义程度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=9;
    ...
    ret+="该帖阅读后心得价值"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖恶劣程度为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖恶劣程度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖大快人心的程度为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖情绪利用程度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=9;
    ...
    ret+="该帖阅读后心得价值"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖内容像xxs评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖内容像xxs评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖内容违规力度评价为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖内容违规力度评价为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=9;
    ...
    ret+="该帖阅读后心得价值"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    cout<<"该帖读完后感动程度为？（回复等级，如 A，最高 S，最低 C）\n\n";
    getstring();
    ret+="该帖读完后感动程度为"+g[grfr(v)]+"\n\n",all+=po[grfr(v)];
    system("cls");
    all/=7;
    ...
    ret+="**综合评价";
    if(all>=6) ret+=g[1];
    else if(all>=3) ret+=g[2];
    else if(all>=1.5) ret+=g[3];
    else ret+=g[4];
    ret+="**\n\n";
  }
  else if(op==2){
    ...
    if(op<=2&&op>=1){
      ret+="该帖内容丰富度评价为";
      if(op==1) ret+=g[4],all+=1;
      else{
        system("cls");
        cout<<"该帖有观点、理由、证据三个要素中的几个？（回复数字）\n【1】1 个\n【2】2 个\n【3】3 个\n【4】0 个\n\n";
        get();
        if(op<=4&&op>=1){
          if(op==4) ret+=g[4],all+=1;
          if(op==1) ret+=g[3],all+=2;
          if(op==2) ret+=g[2],all+=4;
          if(op==3) ret+=g[1],all+=7;
        }
        else END;
      }
    }
    ...
    if(op<=3&&op>=1){
      ret+="该帖文本表达评价为";
      if(op==3) ret+=g[4],all+=1;
      else if(op==2) ret+=g[3],all+=2; 
      else{
        system("cls");
        cout<<"该帖艺术手法效果明显吗？（回复数字）\n【1】效果明显\n【2】有点效果\n【3】用了艺术手法但和没用一样\n【4】艺术手法都没有\n\n";
        
		get();
        if(op<=4&&op>=1){
          if(op==4) ret+=g[4],all+=1;
          if(op==1) ret+=g[1],all+=7;
          if(op==2) ret+=g[2],all+=4;
          
          if(op==3) ret+=g[3],all+=2;
        }
        else END;
      }
    ...
    ret+="\n\n";
    system("cls");
    cout<<"该帖的写法你有印象吗？（回复数字）\n【1】有\n【2】无\n\n";
    get();
    if(op<=2&&op>=1){
      ret+="该帖创新性评价为";
      system("cls");
      if(op==1){
        cout<<"和你印象中的差别大不大？（回复数字）\n【1】有些差别\n【2】简直一模一样\n\n";
        get();
        if(op<=2&&op>=1){
          if(op==1) ret+=g[3],all+=2;
          if(op==2) ret+=g[4],all+=1;
  ...
  if(vop==6){
    ...
    if(all>=7) id=" S";
      ...
      else if(all>=1.5) id=" B";
      else if(all>=0.5) id=" C";
      else id=" D";
    cout<<"总分为 "<<all<<"，等级为"+id+"，自定义结尾句。\n\n";
    getstring();
    ret+=v+"\n";
  }
  else{
    ...
    if(op==1){
      ...
      if(all>=3){
        if(vop==1) ret+="## 楼主 JC 实力可以，期待下次的表演。（手动狗头）\n";
        if(vop==2) ret+="## 再怎么说，都是无意义。\n";
        if(vop==3) ret+="## 楼主 NB，再来几个！（十分赞许）\n";
        if(vop==4) ret+="## 好，但是违规了，建议删除帖子。\n";
        if(vop==5) ret+="## 感动。\n";
      }
      else{
        if(vop==1) ret+="## 楼主 JC 实力一般，希望下次能带给我们更好观感。\n";
        if(vop==2) ret+="## 楼主帖子质量低迷且无意义，希望提升质量。\n";
        if(vop==3) ret+="## 楼主 NB，但你这实力不行啊，回炉重造一下吧。\n";
        if(vop==4) ret+="## 帖子质量低，还请不要再违规了。xxs警告！\n";
        if(vop==5) ret+="## 是真情实感吗？\n";
      }
    }
    else END;
  }
  ret+="\n_BY [HKJC 质量评价生成器 内部版 "+version+"](https://www.luogu.com.cn/blog/Mr-hacker/HKJC-NC)_\n";
  system("cls");
  cout<<"接下来是预览！\n\n--------------------\n\n";
  cout<<ret<<"\n";
  Copy(ret); 
  Sleep(7*1000);
  system("cls");
  cout<<"已为您自动复制，感谢会员"<<bu<<"的测试！";
  system(">nul 2>nul pause");
  cout<<endl<<"--------------------------------"<<endl<<"Process exited after 6.911 seconds with return value 0"<<endl;
  cout<<"请按任意键继续. . .";
  Sleep(10*1000);
  system("cls");
  int ass; 
  system("cls");
  cout<<"请问需要休闲一下吗？【1】需要 【2】不需要\n\n";
  cout<<"尊贵的欧皇会员，请您输入>>>";
  cin>>ass;
  system("cls");
  if(ass==1){
  	SetConsoleTitle("那么，休闲一下！");
	SetWindowLongPtrA(GetConsoleWindow(),GWL_STYLE,GetWindowLongPtrA(GetConsoleWindow(),GWL_STYLE)&~WS_SIZEBOX&~WS_MAXIMIZEBOX&~WS_MINIMIZEBOX);
  	MessageBox(NULL,(LPCTSTR)TEXT("O(∩_∩)O哈哈~ 你居然还没有被我骗到！"), (LPCTSTR)TEXT("HKJC 欧皇MAX 彩蛋来喽！"), MB_OK);
	...
	if(Blo<=0){
		...
		Setpos(30,0);
		system("cls");
		cout<<"恭喜你，你已经获得彩蛋了，对我（UID：907233 昵称：一只小菜包）发送口令：UsdPitle WIN caidan927 +（当天日期，如24.12.29） ";
		system("cls");
		cout<<endl<<"--------------------------------"<<endl<<"Process exited after 6.917 seconds with return value 0"<<endl;
  cout<<"请按任意键继续. . .";
